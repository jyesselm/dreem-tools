...
def runmulti(csv, data_dir, seq_path):
    ...
    pos = 0
    for i, row in df.iterrows():
        name, code, barcode_seq, type, align_type = \
            row['name'], row['code'], row['barcode_seq'], row['type'], row['align_type']
        ...
        if row['align_type'] == "NORM":
            ...
            cmd += '--map_score_cutoff 25 --bt2_alignment_args "--local;--no-unal;--no-mixed;--no-discordant;-i S,1,0.75;' \
